...
dataset = '1000'
dataset_dir = '../data/openlane/openlane/images/'
data_dir = '../data/openlane/openlane/lane3d_1000/'
...
ms2one=dict(
    type='DualAttentionFusion',
    inc=_dim_, outc=_dim_, num_scales=4,
    use_spatial=True, use_channel=True)

transformer=dict(
    type='GeoCNetTransformer',
    decoder=dict(
        type='GeoCNetTransformerDecoder',
        embed_dims=_dim_,
        num_layers=6,
...
sparse_ins_decoder=Config(
    dict(
        encoder=dict(
            out_dims=_dim_),
        decoder=dict(
            num_query=geocnet_cfg['num_query'],
            num_group=geocnet_cfg['num_group'],
            sparse_num_group=geocnet_cfg['sparse_num_group'],
            hidden_dim=_dim_,
            kernel_dim=_dim_,
