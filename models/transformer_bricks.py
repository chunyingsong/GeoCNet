...
class GeoCNetDecoderLayer(BaseTransformerLayer):
    def __init__(self,
                 attn_cfgs,
                 ...
                 ffn_num_fcs=2,
                 **kwargs):
        super(GeoCNetDecoderLayer, self).__init__(
            attn_cfgs=attn_cfgs,
...
class GeoCNetTransformerDecoder(TransformerLayerSequence):
    def __init__(self,
                 *args, embed_dims=None,
                 ...
                 split_point=0.3,
                 **kwargs):
        super(GeoCNetTransformerDecoder, self).__init__(*args, **kwargs)
    ...
    def pred2M(self, pitch_z):
        ...
        near_t = pitch_z[:, 0] / 100  
        near_z = pitch_z[:, 1]        
        far_t = pitch_z[:, 2] / 100   
        far_z = pitch_z[:, 3]        
        
        one = torch.ones_like(near_t)
        zero = torch.zeros_like(near_t)
        
        M_near = torch.stack([
            one, zero, zero, zero,
            zero, near_t.cos(), -near_t.sin(), zero,
            zero, near_t.sin(), near_t.cos(), near_z,
            zero, zero, zero, one], dim=-1).view(near_t.shape[0], 4, 4)
            
        M_far = torch.stack([
            one, zero, zero, zero,
            zero, far_t.cos(), -far_t.sin(), zero,
            zero, far_t.sin(), far_t.cos(), far_z,
            zero, zero, zero, one], dim=-1).view(far_t.shape[0], 4, 4)
            
        return M_near, M_far

    def smooth_transition(self, near_mask, width=2.0):
        transition = torch.sigmoid(width * near_mask)
        
        transition = torch.clamp(transition, min=0.0, max=1.0)
        return transition
    ...
    def forward(self, query, key, value,
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if layer_idx < len(self.layers) - 1:
                ...
                pred = self.gflat_pred_layer(input_feat).squeeze(-1).squeeze(-1)  # [B, 4]
                M_near, M_far = self.pred2M(pred)  # [B, 4, 4]
                
                points_y = init_ref_3d_homo[..., 1]  # [B, 150, 70]
                y_min = points_y.min()
                y_max = points_y.max()
                y_range = y_max - y_min
                
                point = y_min + self.split_point * y_range

                ref_3d_homo_new = init_ref_3d_homo.clone()
                bs = init_ref_3d_homo.shape[0]
                for b in range(bs):
                    b_near_mask = (points_y[b] < point)
                    b_far_mask = ~b_near_mask
                    
                    b_M_near = M_near[b]  # [4, 4]
                    b_M_far = M_far[b]    # [4, 4]
                    
                    if b_near_mask.any():
                        b_near_points = init_ref_3d_homo[b, b_near_mask].view(-1, 4)  # [N_near, 4]
                        b_near_transformed = torch.matmul(b_near_points, b_M_near.transpose(0, 1))
                        ref_3d_homo_new[b, b_near_mask] = b_near_transformed
                    
                    if b_far_mask.any():
                        b_far_points = init_ref_3d_homo[b, b_far_mask].view(-1, 4)    # [N_far, 4]
                        b_far_transformed = torch.matmul(b_far_points, b_M_far.transpose(0, 1))
                        ref_3d_homo_new[b, b_far_mask] = b_far_transformed
                  
                ref_3d_homo = ref_3d_homo_new
...
class GeoCNetTransformer(BaseModule):
    def __init__(self, encoder=None, decoder=None, init_cfg=None):
        super(GeoCNetTransformer, self).__init__(init_cfg=init_cfg)
