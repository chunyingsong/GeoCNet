...
from .geocnet_head import GeoCNetHead
...
class GeoCNet(nn.Module):
    def __init__(self, args):
        ...
        _dim_ = args.geocnet_cfg.fpn_dim
        num_query = args.geocnet_cfg.num_query
        num_group = args.geocnet_cfg.num_group
        sparse_num_group = args.geocnet_cfg.sparse_num_group

        self.encoder = build_backbone(args.geocnet_cfg.encoder)
        if getattr(args.geocnet_cfg, 'neck', None):
            self.neck = build_neck(args.geocnet_cfg.neck)
        else:
            self.neck = None
        ...
        self.head = GeoCNetHead(
            ...
            **args.geocnet_cfg.get('head', {}),
            trans_params=args.geocnet_cfg.get('trans_params', {})
        )
