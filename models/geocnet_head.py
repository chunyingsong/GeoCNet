...
from .curve import CurveLoss


class GeoCNetHead(nn.Module):
    def __init__(self, args,
        ...
        # Initialize the curve loss calculator
        self.curve_loss_calculator = CurveLoss(smooth_weight=1.0, continuity_weight=1.0)
    ...
    def get_loss(self, output_dict, input_dict):
        ...
        def single_layer_loss(layer_idx):
            ...
            for b_idx in range(len(matched_indices[0])):
                for group_idx in range(self.num_group):
                    ...
                    smooth_loss , continuity_loss = self.curve_loss_calculator(
                        pred_xs, pred_zs, gt_xs, gt_zs, self.anchor_y_steps, loc_mask
                    )
                    per_smooth_loss = smooth_loss
                    per_continuity_loss = continuity_loss
            ...
            return tuple(map(lambda x: x / batch_size / self.num_group,
                             [per_xs_loss, per_zs_loss, per_vis_loss, per_cls_loss,per_smooth_loss, per_continuity_loss]))

        all_xs_loss, all_zs_loss, all_vis_loss, all_cls_loss, all_smooth_loss ,all_continuity_loss = multi_apply(
        ...
        all_smooth_loss = sum(all_smooth_loss) / num_layers  
        all_continuity_loss = sum(all_continuity_loss) / num_layers  
        return dict(
            ...
            all_smooth_loss=self.smooth_loss_weight * all_smooth_loss,  
            all_continuity_loss=self.continuity_loss_weight * all_continuity_loss,  
        )
